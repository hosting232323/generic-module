<template>
	<v-container>
		<v-row align="center">
			<v-col v-if="isLeftAligned" cols="12" md="6" sm="6">
			...
			<v-col cols="12" md="6" sm="6">
				<v-sheet :color="info.secondaryColor">
					<h1 v-if="content.title"
						:style="{ color: info.primaryColor + ' !important' }" v-html="content.title" /><br>
					<h1 v-if="content.subtitle" class="text-subtitle-1 font-weight-black"
						v-html="content.subtitle + '<br>'" />
					<v-typography v-if="content.description" v-html="content.description" />
					<br>
					<a :href="content.url" v-if="content.url && content.button">
						<v-btn v-html="content.button" class="text-none" variant="flat" :color="info.primaryColor"
							:style="{ marginTop: '7px' }">
						</v-btn>
					</a>
				</v-sheet>
